...
export function RecipientsList() {
  ...
  const sorted = useMemo(() => {
    ...
    withIndex.sort((a, b) => {
      ...
      } else if (sortKey === "last_correspondence") {
        result = (left.last_correspondence_date ?? "").localeCompare(
          right.last_correspondence_date ?? "",
        );
  ...
  const updateSort = (nextKey: SortKey) => {
    ...
    const defaultDir: SortDir =
      nextKey === "updated" || nextKey === "last_correspondence"
        ? "desc"
        : "asc";
    if (sortKey !== nextKey) {
              ...
              <tr>
                ...
                <th className="px-3 py-2">
                  <button
                    type="button"
                    className="text-left"
                    onClick={() => updateSort("last_correspondence")}
                  >
                    Last correspondence{sortIndicator("last_correspondence")}
                  </button>
                </th>
                <th className="px-3 py-2">
                  <button
