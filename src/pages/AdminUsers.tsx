...
const roles: Role[] = ['admin', 'contacts_manager', 'delivery_coordinator', 'view_only']
...
export function AdminUsers() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const haystack = `${r.full_name ?? ''} ${r.email ?? ''} ${r.id} ${r.role}`.toLowerCase()
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-neutral-600">Assign roles for team members.</p>
      </div>
                ...
                <tr key={r.id} className="border-t border-neutral-200">
                  ...
                  <td className="px-3 py-2">
                    <select
                      ...
                      value={r.role}
                      onChange={(e) => void setRole(r.id, e.target.value as Role)}
