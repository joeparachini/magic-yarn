...
const statusOptions: DeliveryStatus[] = ['pending', 'scheduled', 'completed', 'cancelled']
...
export function DeliveryEdit() {
  ...
  const [addressSource, setAddressSource] = useState<'organization' | 'contact'>('organization')

  const [form, setForm] = useState<DeliveryFormState>(emptyForm())
  const [items, setItems] = useState<DeliveryItemRow[]>([])

  const title = useMemo(() => (isNew ? 'New delivery' : 'Edit delivery'), [isNew])

  const update = (patch: Partial<DeliveryFormState>) => setForm((prev) => ({ ...prev, ...patch }))
  ...
  const loadContactsForOrg = async (organizationId: string) => {
    ...
    const { data, error } = await supabase
      .from('contacts')
      .select('id, first_name, last_name, organization_id, address, city, state, zip')
  ...
  const formatAddress = (parts: {
    ...
    const line2 = [parts.city, parts.state, parts.zip].filter(Boolean).join(', ').replace(', ,', ',').trim()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from('deliveries')
        .select('organization_id, contact_id, delivery_date, status, coordinator_id, address, notes')
        .eq('id', id)
        .maybeSingle()
      ...
      if (existingAddress && contactAddr && existingAddress === contactAddr) setAddressSource('contact')
  ...
  const save = async () => {
    ...
    const resolvedAddress = addressSource === 'contact' ? contactAddress : orgAddress
    if (!resolvedAddress) {
      setError(
    ...
    if (isNew) {
      const { data, error } = await supabase.from('deliveries').insert(payload).select('id').maybeSingle()
      if (error) {
        setError(error.message)
        setSaving(false)
        return
      ...
      navigate(createdId ? `/deliveries/${createdId}` : '/deliveries', { replace: true })
      setSaving(false)
      return
    }
  ...
  const assignToMe = async () => {
    ...
    if (!isNew) {
      ...
      const { error } = await supabase
        .from('deliveries')
        .update({ coordinator_id: user.id, updated_at: new Date().toISOString() })
        .eq('id', id)
  ...
  const updateItem = async (itemId: string, patch: Partial<Pick<DeliveryItemRow, 'wig_type' | 'quantity' | 'notes'>>) => {
    if (!canEdit) return
    setSaving(true)
    setError(null)
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="flex flex-col gap-2 md:col-span-2">
              ...
              <select
                ...
                onChange={(e) => update({ organization_id: e.target.value, contact_id: '' })}
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              <label className="text-xs font-medium text-neutral-700">Contact</label>
              <select
                className="rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm"
              ...
              {!form.organization_id ? (
                <div className="text-xs text-neutral-600">Select an organization first.</div>
              ) : null}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                className="rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm"
                value={form.status}
                onChange={(e) => update({ status: e.target.value as DeliveryStatus })}
                disabled={!canEdit || saving}
              >
                {statusOptions.map((s) => (
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              <label className="text-xs font-medium text-neutral-700">Coordinator</label>
              {isAdmin ? (
                <select
                  className="rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm"
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    value={form.coordinator_id ? (form.coordinator_id === user?.id ? 'Assigned to you' : form.coordinator_id) : 'Unassigned'}
                    readOnly
                  />
                  {canEdit ? (
                    <Button variant="secondary" onClick={() => void assignToMe()} disabled={saving}>
                      Assign me
                    </Button>
                  ) : null}
                </div>
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              ...
              <div className="flex flex-col gap-2 rounded-md border border-neutral-200 p-3">
                ...
                <div className="text-xs text-neutral-600">
                  Address is restricted to the selected Organization or Contact address.
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-neutral-600">Wig types and quantities.</p>
              </div>
              {canEdit ? (
                <Button variant="secondary" onClick={() => void addItem()} disabled={saving}>
                  Add item
                </Button>
              ) : null}
            </div>
                    ...
                    <tr key={it.id} className="border-t border-neutral-200">
                      <td className="px-3 py-2">
                        <input
                          ...
                          value={it.wig_type}
                          onChange={(e) =>
                            setItems((prev) => prev.map((p) => (p.id === it.id ? { ...p, wig_type: e.target.value } : p)))
                          }
                          onBlur={() => void updateItem(it.id, { wig_type: it.wig_type })}
                          disabled={!canEdit || saving}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          ...
                          onChange={(e) =>
                            setItems((prev) =>
                              prev.map((p) =>
                                p.id === it.id ? { ...p, quantity: Number(e.target.value || 1) } : p,
                              ),
                            )
                          }
                          onBlur={() => void updateItem(it.id, { quantity: it.quantity })}
                          disabled={!canEdit || saving}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          ...
                          value={it.notes ?? ''}
                          onChange={(e) =>
                            setItems((prev) => prev.map((p) => (p.id === it.id ? { ...p, notes: e.target.value } : p)))
                          }
                          onBlur={() => void updateItem(it.id, { notes: (it.notes ?? '').trim() || null })}
                          disabled={!canEdit || saving}
                        />
                      </td>
                      <td className="px-3 py-2 text-right">
                        {canEdit ? (
                          <Button variant="secondary" onClick={() => void deleteItem(it.id)} disabled={saving}>
                            Delete
                          </Button>
                        ) : null}
                      </td>
            ...
            {isNew ? <div className="text-xs text-neutral-600">Save the delivery to manage items.</div> : null}
          </div>
        </div>
      )}
    </div>
