...
function MetricCard({ title, value, hint }: { title: string; value: string; hint?: string }) {
  return (
    <div className="rounded-lg border border-neutral-200 p-4">
      <div className="text-xs font-medium uppercase tracking-wide text-neutral-600">{title}</div>
...
async function hasPermission(permission: Permission): Promise<boolean> {
  const { data, error } = await supabase.rpc('current_user_has_permission', { p: permission })
...
export function Dashboard() {
  ...
  const [statusCounts, setStatusCounts] = useState<Record<DeliveryStatus, number> | null>(null)
  ...
  const load = async () => {
    ...
    try {
      ...
      if (deliveriesOk) {
        ...
        tasks.push(
          (async () => {
            const statuses: DeliveryStatus[] = ['pending', 'scheduled', 'completed', 'cancelled']
            ...
            setStatusCounts(Object.fromEntries(results) as Record<DeliveryStatus, number>)
          })(),
        )
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-600">Quick overview and recent updates.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          title="Organizations"
          value={canReadOrgs ? (orgCount ?? (loading ? '…' : '0')).toString() : '—'}
          hint={canReadOrgs ? 'Total organizations' : 'No access'}
        />
        <MetricCard
          title="Contacts"
          value={canReadContacts ? (contactCount ?? (loading ? '…' : '0')).toString() : '—'}
          hint={canReadContacts ? 'Total contacts' : 'No access'}
        />
        <MetricCard
          title="Deliveries"
          value={canReadDeliveries ? (deliveryCount ?? (loading ? '…' : '0')).toString() : '—'}
          hint={canReadDeliveries ? deliveryHint : 'No access'}
        />
        <MetricCard
          title="Next 14 Days"
          value={canReadDeliveries ? (upcomingCount ?? (loading ? '…' : '0')).toString() : '—'}
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-neutral-200 p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <MetricCard title="Pending" value={pending === null ? '—' : pending.toString()} />
              <MetricCard title="Scheduled" value={scheduled === null ? '—' : scheduled.toString()} />
              <MetricCard
                title="Completed"
                ...
                <tbody>
                  {recentDeliveries.map((d) => (
                    <tr key={d.id} className="border-b border-neutral-100">
                      ...
                      <td className="py-2 pr-3">{d.organizations?.name ?? '—'}</td>
                      <td className="py-2 pr-3">
                        {d.contacts ? `${d.contacts.last_name}, ${d.contacts.first_name}` : '—'}
                      </td>
                    </tr>
                  ))}
                  {recentDeliveries.length === 0 ? (
                    <tr>
                      <td className="py-6 text-center text-sm text-neutral-600" colSpan={4}>
                        No deliveries yet.
                      </td>
                    </tr>
                  ) : null}
